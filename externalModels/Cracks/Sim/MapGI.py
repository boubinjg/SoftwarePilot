...
if __name__ == '__main__':
    ...
    sampleVec = [i for i in range(int(len(imagedata)/4*3)+1, len(imagedata))]
    ...
    #count = len(giMap)
    #mean = statistics.mean(giMap)
    #std = statistics.stdev(giMap)
    #print([mean, std, min(giMap), max(giMap), count])
