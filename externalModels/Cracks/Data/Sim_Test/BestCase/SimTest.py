...
def findNext(image, fieldmap, oldErr, knndata):
    ...
    nbors.fit(knndata[:,0:15])

    knn = nbors.kneighbors([query[0:15]])
    knn = knn[1][0]
...
def crackedpct(img):
    return float(img[12].split(',')[14].split('=')[1][:-1])

def getNeighbors(image):
    return image[4:8]

def dls(image, runs, visited):
    imgNum = image[1]
    cp = crackedpct(image)
    neighbors = getNeighbors(image)
    nvis = copy.deepcopy(visited)
    nvis.append(imgNum)

    if(runs > 0):
        rets = []
        for neighbor in neighbors:
            if(neighbor not in nvis and neighbor != '[]'):
                index = neighbor
                next = getImage(index)
                val = dls(next, runs-1, nvis)
                rets.append(val)
        if(rets != []):
            return cp+max(rets)
