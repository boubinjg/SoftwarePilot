 ...
 * @since   2018-6-25
 */
public class BasicVision extends org.reroutlab.code.auav.routines.AuavRoutines {
		/**
		...
		public void run() {
            ...
            config(); //configures camera and flight functions
            ...
            takeImg(true); //capture 4k image
            pic = read4k(); //read 4k image from VM into local memory

            //there is a bettery way to do this
            succ = invokeHostDriver("dn=VisionDriver-dc=fce", IP, auavResp.ch, true);

            try{
                sendToPort(pic, IP, 12013);
            } catch(Exception e){
                e.printStackTrace();
            }

            System.out.println(auavResp.getResponse());
            if(!auavResp.getResponse().equals("[]")) {
                succ = invokeDriver("org.reroutlab.code.auav.drivers.DroneGimbalDriver", "dc=shk", auavResp.ch);
            }
		...
		public String startRoutine() {
				if (t != null) {
						t.start(); return "BasicVision: Started";
				}
				return "Detect not Initialized";
		}
		public String stopRoutine() {
				forceStop = true;	return "BasicVision: Force Stop set";
		}
}
