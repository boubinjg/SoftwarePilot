...
class Parser {
    ...
    public String getFeature(String line, int num) throws Exception{
        ...
        //System.out.println(script.get(num) + " Spawning c/md "+num);
    ...
    public void getUtility(ArrayList<String> features){
        ...
        for(String feature : features){
            ...
            if(featureType.equals("corn")) {
                ...
                if(bor.equals(""))
                    continue;
                utility += .04 * Math.abs(Double.parseDouble(featureVal) -
                                          Double.parseDouble(bor.split("=")[1]));
                constants += .04;
            } else if(featureType.equals("BrightnessTotal")) {
                utility += .015 * Double.parseDouble(featureVal);
                constants += .015;
            } else if(featureType.equals("time")) {
                double sec = Double.parseDouble(featureVal)*24.0;
                if(sec >= .5)
                    utility += .05 * (24 - 2.0*(sec - 12))/24.0;
                else
                    utility += .05 * (2.0*sec)/24.0;

                constants += .05;
            } else if(featureType.equals("GreenIndex")) {
		double gi = Double.parseDouble(featureVal)*10;
		utility += .5 * gi;
		constants += .5;
	    }
        }
